...
{
    // Delete Series
    public class CreateCommandDeleteSeries : ICommandFactory
    {
        public string commandName => "delete series";
        ...
        public ICommand CreateCommand(string commandText, Dictionary<string, object> d)
        {
            ...
            command = new CommandDeleteSeries(d, commandText, requirementsList);

            return command;
        }
    }

    // Delete series base - command
    public class CommandDeleteSeries : ICommand
    {
        public Dictionary<string, object> d;
        List<(string, string, char)> requirementsList;
        List<Series> collection;
        int deleteCounter = 0;
        List<(Series,int)> collectionRestricted = new List<(Series,int)>();
        public string CommandName { get; }

        public CommandDeleteSeries(Dictionary<string, object> d, string commandText, List<(string, string, char)> requirementsList)
        {
            this.d = d;
